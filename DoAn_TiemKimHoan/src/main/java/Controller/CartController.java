...
public class CartController {
	...
	@RequestMapping(value = "EditCart/{id}/{quanty}.htm")
	...
	public String DeleteCart(HttpServletRequest request, HttpSession session, @PathVariable
			String id) {
		HashMap<String, Cart> cart = (HashMap<String, Cart>) session.getAttribute("Cart");
		if (cart == null) {
			cart = new HashMap<String, Cart>();
		...
		cart.forEach((key, value) -> {
		      System.out.println("day la hashmap: //" + key +"//");
		    });
		System.out.println("aaaaa_"+id+"_");
		cart = cartService.DeleteCart(id.trim(), cart);
		session.setAttribute("Cart", cart);
		session.setAttribute("TotalQuantyCart", cartService.TotalQuanty(cart));
		session.setAttribute("TotalPriceCart", cartService.TotalPrice(cart));
