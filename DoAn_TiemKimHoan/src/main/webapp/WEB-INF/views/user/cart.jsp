		...
		<div class="container">
			<div id="content" class="full">
					...
					<table>
						...
						<c:forEach var="item" items="${ Cart }">
							<tr>
								<td class="items">
									<div class="image">
										<a href="<c:url value ='detail${item.value.product.id }.htm?size=${item.value.product.size }'/>">
										<img
									...
									<h3>
										<a href="<c:url value ='detail${item.value.product.id }.htm?size=${item.value.product.size }'/>">${ item.value.product.name }</a>
									</h3>
				...
				<div class="total-count">
					...
					<c:if test="${empty a  }">
					<a href="login1.htm" class="btn-grey">Đăng nhập</a>
					</c:if>
					<c:if test="${not empty a && empty Cart}">
					<a href="products.htm?index=1" class="btn-grey">Mua hàng</a>
					</c:if>
					<c:if test="${not empty a && not empty Cart }">
					<a href="checkout.htm" class="btn-grey">Đặt hàng</a>
					</c:if>
...
<h1>${success }</h1>
	...
	<content tag="script"> <script>
		$(".edit-cart").on("click", function() {
			...
			window.location = "EditCart/" + id + "/" + quanty+".htm";
		...
		$(".delete-cart").on("click", function() {
			...
			//confirm("Bạn có chắc chắn muốn xóa.");
			 if(confirm("Bạn có chắc chắn muốn xóa. Chọn OK để XÓA") == true){
				 window.location = "DeleteCart/"+id+".htm";
