			...
			<div class="last-products">
				...
				<section class="products">
					<c:forEach items="${listP}" var="o">
						<article>
							<a href="<c:url value ='detail${o.id }?size=${o.size }'/>"><img
								src="<c:url value ='/resources/images/${o.image }'/>" alt=""></a>
							<h3>
								<a href="<c:url value ='detail${o.id }?size=${o.size }'/>">${o.name }</a>
							</h3>
							<h4>
								<a class="colorvang"
									href="<c:url value ='detail${o.id }?size=${o.size }'/>"><fmt:formatNumber
										type="number" groupingUsed="true" value="${ o.price }" /> đ</a>
							</h4>
							<c:if test="${not empty a  }">
								<a href="<c:url value ='AddCart/${o.id }'/>" class="btn-add">ADD
									TO CART</a>
							</c:if>
							<c:if test="${empty a  }">
